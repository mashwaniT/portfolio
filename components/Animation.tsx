...
function Content() {
  const [springs, api] = useSprings(number, (i) => ({
    from: random(i),
    ...random(i),
    config: { mass: 30, tension: 100, friction: 70 }
  }))
  

  useEffect(() => void setInterval(() => api.start((i) => ({ ...random(i), delay: i * 60 })), 5000), [])
...
export default function Animation({ className }: AnimationProps) {
  return (
    <div className={`body rounded-lg h-72 w-72 ${className}`}>
